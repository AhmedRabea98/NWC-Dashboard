...
namespace NWC_Dashboard.Controllers
{
    ...
    public class RegionController : ControllerBase
    {
        ...
        public RegionController(ApplicationDBContext context , ApplicationDBContext2 context2)
        {
        ...
        {
            var proInfo = context.UnifierUsPs.Where(e => e.FypNwcRegion.ToLower() == region.ToLower()).ToList();
            var PaymentNew = context.UnifierUxpas.Where(e => e.FypNwcCity.StartsWith(region.ToUpper())).ToList();
            var changePending = context.UnifierUcoxes.Where(e => e.FypNwcCity.StartsWith(region.ToUpper())).ToList();
            var project = context2.Projects.ToList();
            var projectX = context2.Projectxes.ToList();

            DataByRegion data = new DataByRegion();
