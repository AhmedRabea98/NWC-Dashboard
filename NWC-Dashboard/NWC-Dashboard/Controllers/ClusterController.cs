...
namespace NWC_Dashboard.Controllers
{
    ...
    public class ClusterController : ControllerBase
    {
        ...
        public ClusterController(ApplicationDBContext context , ApplicationDBContext2 context2)
        {
        ...
        public IActionResult Get(string cluster)
        {
            ...
            var proInfo = context.UnifierUsPs.Where(e=>e.FypNwcCluster.ToLower() == cluster.ToLower()).ToList();
            var PaymentNew = context.UnifierUxpas.Where(e => e.Ugenclustertname.StartsWith(cluster.ToUpper())).ToList();
            var changePending = context.UnifierUcoxes.Where(e => e.Ugenclustertname.StartsWith(cluster.ToUpper())).ToList();
            var project = context2.Projects.ToList();
            var projectX = context2.Projectxes.ToList();

            DataByCluster data = new DataByCluster();

            //Main Data
            data.Regions = proInfo.Select(e => e.FypNwcRegion).Distinct().ToList();
